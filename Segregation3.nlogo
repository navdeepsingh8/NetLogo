globals [
  ...
  percent-full     ;; how many tiles are taken around a turtle on average
]
...
turtles-own [
  ...
  individual-%-similar-wanted ;; actual percentage of similar individuals wanted
  individual-%-full-wanted    ;; actual percentage of neighbours wanted
]
...
to setup-turtles
  ask turtles [
    determine-desired-similarity
    determine-desired-neighbours
  ]
end  

to determine-desired-similarity
  set individual-%-similar-wanted  ( random-normal %-similar-wanted %-similar-deviation )
  if ( individual-%-similar-wanted > 100 ) [ determine-desired-similarity ] ;; retry if similarity over 100%
  if ( individual-%-similar-wanted < 0 ) [ determine-desired-similarity ] ;; retry if similarity under 0%
end

to determine-desired-neighbours
  set individual-%-full-wanted ( random-normal %-full-wanted %-full-deviation )
  if ( individual-%-full-wanted > 100 ) [ determine-desired-neighbours ]
  if ( individual-%-full-wanted < 0   ) [ determine-desired-neighbours ]
end
...
to update-turtles
  ask turtles [
    ...
    set happy? (similar-nearby >= ( individual-%-similar-wanted * total-nearby / 100 )) and (total-nearby >= (individual-%-full-wanted / 100 * 8))
  ]
end
...
to update-globals
  ...
  set percent-full (total-neighbors / (count turtles * 8)) * 100
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot percent-full"
